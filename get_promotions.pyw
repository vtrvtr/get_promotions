...
def get_soup(link):
    ...
    return bs(html_doc.content, 'html5lib')
...
def format_adrenaline_link(link):
    ...
    _, title = title.split(b'>', 1)
    thread_title, _ = title.split(b'<', 1)
    return "\nPromo: {}\nLink: {}\n".format(thread_title, link) if thread_title.startswith(b' [') else None
...
#     get_hardmob_links()
#     Popen(['notepad.exe', '{}'.format(FILE_PATH)])
